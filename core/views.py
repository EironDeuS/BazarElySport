...
def inicio(request):
    registros = []  # Inicializa registros antes de las condiciones
...
def ficha(request, producto_id):
    ...
    productos_relacionados = Producto.objects.filter(categoria=producto.categoria).exclude(id=producto.id)
...
def ventas(request):
    boletas = Boleta.objects.all()  # Usar `Boleta` en lugar de `boleta`
    historial = []
...
def productos(request, accion, id):
    form = None  # Inicializar la variable form
    ...
    if request.method == 'POST':
        ...
        elif accion == 'eliminar' or request.POST.get('accion') == 'eliminar':  # Aquí se maneja la eliminación
            exito, mensaje = eliminar_registro(Producto, id)  # Pasar el modelo y el id
            messages.success(request, mensaje)
            if exito:
                return redirect(reverse('productos', kwargs={'accion': 'crear', 'id': 0}))

        if form and form.is_valid():
            producto = form.save()
            accion_realizada = "actualizado" if accion == "actualizar" else "creado"
            messages.success(request, f'Producto "{str(producto)}" {accion_realizada} exitosamente')
            return redirect(reverse('productos', kwargs={'accion': 'actualizar', 'id': producto.id}))
        else:
            if form:
                show_form_errors(request, [form])
    ...
    # Asegurarse de que form se pase en el contexto
    context = {
        'form': form,
...
def obtener_productos(request):
    ...
    if not categoria_id:
        return JsonResponse({'error': 'categoria_id no proporcionada'}, status=400)
...
def agregar_producto_al_carrito(request, producto_id):
    if es_personal_autenticado_y_activo(request.user):
        ...
        return redirect('inicio')  # Asegúrate de que 'inicio' sea la URL name correcta
    elif es_usuario_anonimo(request.user):
        messages.info(request, 'Para poder comprar, primero debes registrarte como cliente.')
        return redirect('registrarme')  # Asegúrate de que 'registrarme' sea la URL name correcta
    ...
    return redirect('ficha', producto_id=producto_id)

@user_passes_test(es_cliente_autenticado_y_activo)
def eliminar_producto_en_carrito(request, carrito_id):
    Carrito.objects.get(id=carrito_id).delete()
    messages.success(request, "El producto ha sido eliminado del carrito exitosamente.")
